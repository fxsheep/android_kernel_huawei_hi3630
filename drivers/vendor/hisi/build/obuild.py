...
def main():
	...
	#ccache
	use_ccache = "1"
	if params_dict.has_key('COVERITY') and params_dict['COVERITY']!='' :
		use_ccache = ""
	if params_dict.has_key('FORTIFY') and params_dict['FORTIFY']!='' :
		use_ccache = ""
	os.environ['USE_CCACHE'] = use_ccache
	...
	#		os.chmod(product_config_path + "balong_product_config.mk", stat.S_IWRITE)
		...
		if params_dict.has_key('FORTIFY') and params_dict['FORTIFY'] == 'true' :
			...
			if os.path.exists(fortify_path):
				os.system("rm -rf "+fortify_path)
		...
		if params_dict.has_key('COVERITY') and params_dict['COVERITY']!='' :
			coverity_path = top_dir + "/build/delivery/" + product_name + "/coverity"
			if os.path.exists(coverity_path):
				os.system("rm -rf "+coverity_path)
			os.environ['COVERITY_UNSUPPORTED'] = '1'
			coverity_tmp_dir = top_dir + "/build/delivery/" + product_name + "/coverity_tmp"
			os.mkdir(coverity_tmp_dir)
			cmd = top_dir + "/build/tools/coverity/bin/cov-build --dir " + coverity_tmp_dir + " --encoding GB2312 " + cmd
		...
		if params_dict.has_key('FORTIFY') and params_dict['FORTIFY']!='' :
			if os.path.exists(top_dir + '/build/delivery/.fortify/sca6.00/build'):
				list_dir = os.listdir(top_dir + '/build/delivery/.fortify/sca6.00/build')
				for fortify_prj_name in list_dir:
					if os.path.isdir(top_dir + '/build/delivery/.fortify/sca6.00/build/' + fortify_prj_name) and fortify_prj_name.startswith(product_name):
						cmd_generat_fpr = top_dir + "/build/tools/fortify/bin/sourceanalyzer -64 -Xmx10G -b " + fortify_prj_name + " -scan -f " + product_name + ".fpr"
						os.system(cmd_generat_fpr)
				cmd_generat_report = top_dir + "/build/tools/fortify/bin/ReportGenerator -Xmx10G -format xml -f " + product_name + ".xml -source " + product_name + ".fpr -template " + top_dir + "/build/tools/fortify/Core/config/reports/DefaultReportDefinition.xml"
				os.system(cmd_generat_report)
				os.system('rm -r -f ' + top_dir + '/build/' + product_name + '.fpr')
				os.system('rm -r -f ' + top_dir + '/build/delivery/.fortify')
				os.mkdir(fortify_path)
				os.system('mv ' + top_dir + '/build/' + product_name + '.xml ' + fortify_path)
			#add result to buildinfo.txt according to compile result
			if errcode == 0:
				buildinfo +="result:success "
			else:
				buildinfo +="result:fail "
			vsc_info = generate_vsc_info.get_vsc_info(params_dict, additional_pkg_name, top_dir)
			if vsc_info:
				buildinfo += vsc_info
			buildinfo = buildinfo.replace(" ","\n")
			buildinfo = buildinfo.replace("*"," ")
		...
		if params_dict.has_key('COVERITY') and params_dict['COVERITY']!='' :
			cmd_coverity_zip = top_dir + "/build/tools/coverity/bin/7za a -tzip coverity.zip " + coverity_tmp_dir + "/* -r"
			coverity_mid_dir = top_dir + "/build/delivery/" + product_name + "/coverity"
			os.mkdir(coverity_mid_dir)
			os.system(cmd_coverity_zip)
			os.system('mv coverity.zip ' + coverity_mid_dir)
			os.system('rm -r -f ' + coverity_tmp_dir)
			#add result to buildinfo.txt according to compile result
			if errcode == 0:
				buildinfo +="result:success "
			else:
