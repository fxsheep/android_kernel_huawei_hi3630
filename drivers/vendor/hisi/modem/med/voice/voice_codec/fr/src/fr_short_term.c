...
#define STEP(i)    \
        LARp[i] = GSM_ADD( SASR( LARpp_j_1[i], 2 ), SASR( LARpp_j[i], 2 ));\
        LARp[i] = GSM_ADD( LARp[i],  SASR( LARpp_j_1[i], 1));
     ...
     Word16 * LARp)
{
     ...
     CODEC_OpVvFormWithDimi(LARpp_j_1, LARpp_j, 8, LARp);
}
...
static void Coefficients_40_159(
     ...
     Word16 * LARp)
{
     CODEC_OpVecCpy(&LARp[0], &LARpp_j[0], 8);
}

/* 4.2.9.2 */

static void LARp_to_rp(
     Word16 * LARp)   /* [0..7] IN/OUT  */
/*
...
/*
            ...
            tmp2 =  ( tmp1 == MIN_WORD && tmp2 == MIN_WORD
                ? MAX_WORD
                : 0x0FFFF & (( (Word32)tmp1 * (Word32)tmp2
                         + 16384) >> 15)) ;
...
/*
            tmp1  = ( tmp1 == MIN_WORD && sri == MIN_WORD
                ? MAX_WORD
                : 0x0FFFF & (( (Word32)tmp1 * (Word32)sri
                         + 16384) >> 15)) ;
...
{
    Word16        * LARpp_j   = S->LARpp[ S->j      ];
    Word16        * LARpp_j_1 = S->LARpp[ S->j ^= 1 ];
...
#define  FILTER  Short_term_synthesis_filtering
    ...
    Coefficients_0_12( LARpp_j_1, LARpp_j, LARp );
    LARp_to_rp( LARp );
    FILTER(S->v, LARp, 13, wt, s);

    Coefficients_13_26( LARpp_j_1, LARpp_j, LARp);
    LARp_to_rp( LARp );
    FILTER(S->v, LARp, 14, wt + 13, s + 13);

    Coefficients_27_39( LARpp_j_1, LARpp_j, LARp);
    LARp_to_rp( LARp );
    FILTER(S->v, LARp, 13, wt + 27, s + 27);

    Coefficients_40_159( LARpp_j, LARp );
    LARp_to_rp( LARp );
    FILTER(S->v, LARp, 120, wt + 40, s + 40);
}
