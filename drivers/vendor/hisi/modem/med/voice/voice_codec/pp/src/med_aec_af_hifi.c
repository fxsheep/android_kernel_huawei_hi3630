                ...
                MED_AEC_AF_STRU                *pstAf)
{
    ...
    if (CODEC_SAMPLE_RATE_MODE_8000 == enSampleRate)
    {
        ...
        /* 8k采样，每ms采样8点即左移3位 */
        shwAfFilterLen           = shl(pstPara->shwAfTailLen, 3);
    }
    else
    {
...
VOS_VOID MED_AEC_AfSpectralMulAccumUnit(
                VOS_INT16   shwM,
                VOS_INT16   enFftLen,
                VOS_INT16   pshwFarFreq[][MED_AEC_AF_FILT_WB],
                VOS_INT32   pswFilterCoeff[][MED_AEC_AF_FILT_WB],
                VOS_INT32  *pswEchoFreq)
{
    ...
    ae_p24x2s  aepF;
    ae_q56s    aeqE, aeqF, aeqF1, aeqF2;

    /* 4字节对齐保护 */
    if ((((VOS_INT32)&pshwFarFreq[0][0]) & 0x03) != 0)
    {
        return;
    }
    ...
    for (swCntJ = 0; swCntJ < shwM; swCntJ++)
    {
        ...
        pswEchoFreq[0] = CODEC_OpL_add(swTemp1, pswEchoFreq[0]);

        swTemp1        = CODEC_OpL_mpy_32_16(aswFilterCoeff[swCntJ][1],
                                           ashwFarFreq[swCntJ][1]);
        pswEchoFreq[1] = CODEC_OpL_add(swTemp1, pswEchoFreq[1]);
        */
        aepF = *((ae_p16x2s *)&pshwFarFreq[swCntJ][0]);
        aeqF = *((ae_q32s *)&pswFilterCoeff[swCntJ][0]);
        aeqE = *((ae_q32s *)&pswEchoFreq[0]);
        aeqF = AE_MULFQ32SP16S_H(aeqF, aepF);
        aeqE = AE_ADDSQ56S(aeqF, aeqE);
        aeqE = AE_SATQ48S(aeqE);

        pswEchoFreq[0] = AE_TRUNCA32Q48(aeqE);

        aeqF = *((ae_q32s *)&pswFilterCoeff[swCntJ][1]);
        aeqE = *((ae_q32s *)&pswEchoFreq[1]);
        aeqF = AE_MULFQ32SP16S_L(aeqF, aepF);
        aeqE = AE_ADDSQ56S(aeqF, aeqE);
        aeqE = AE_SATQ48S(aeqE);

        pswEchoFreq[1] = AE_TRUNCA32Q48(aeqE);
    ...
    for (swCntI = 2; swCntI < (enFftLen - 1); swCntI = swCntI + 2)
    {
        for (swCntJ = 0; swCntJ < shwM; swCntJ++)
        {
            ...
            pswEchoFreq[swCntI]= CODEC_OpL_add(swTemp1, pswEchoFreq[swCntI]);
            */
            aepF  = *((ae_p16x2s *)&pshwFarFreq[swCntJ][swCntI]);
            aeqE  = *((ae_q32s *)&pswEchoFreq[swCntI]);
            aeqF1 = *((ae_q32s *)&pswFilterCoeff[swCntJ][swCntI]);
            aeqF2 = *((ae_q32s *)&pswFilterCoeff[swCntJ][swCntI + 1]);
            aeqF  = AE_MULFQ32SP16S_H(aeqF1, aepF);
            aeqE  = AE_MULFQ32SP16S_L(aeqF2, aepF);
            aeqF  = AE_SUBSQ56S(aeqF, aeqE);
            aeqE  = *((ae_q32s *)&pswEchoFreq[swCntI]);
            aeqE  = AE_ADDSQ56S(aeqF, aeqE);
            aeqE  = AE_SATQ48S(aeqE);
            pswEchoFreq[swCntI] = AE_TRUNCA32Q48(aeqE);
            ...
            swTemp1 = CODEC_OpL_add(CODEC_OpL_mpy_32_16(aswFilterCoeff[swCntJ][swCntI],
                                  ...
                                  CODEC_OpL_mpy_32_16(aswFilterCoeff[swCntJ][swCntI + 1],
                                                    ashwFarFreq[swCntJ][swCntI]));
            pswEchoFreq[swCntI + 1]= CODEC_OpL_add(swTemp1,pswEchoFreq[swCntI + 1]);
            */
            aeqF = AE_MULFQ32SP16S_L(aeqF1, aepF);
            aeqE = AE_MULFQ32SP16S_H(aeqF2, aepF);
            aeqF = AE_ADDSQ56S(aeqF, aeqE);
            aeqE = *((ae_q32s *)&pswEchoFreq[swCntI + 1]);
            aeqE = AE_ADDSQ56S(aeqF, aeqE);
            aeqE = AE_SATQ48S(aeqE);
            pswEchoFreq[swCntI + 1] = AE_TRUNCA32Q48(aeqE);
        }
                ...
                VOS_INT16  *pshwEchoFreqOut)
{
    ...
    /* 调用优化运算函数 */
    MED_AEC_AfSpectralMulAccumUnit(shwM,
                                   enFftLen,
                                   ashwFarFreq,
                                   aswFilterCoeff,
                                   pswEchoFreq);
...
static inline VOS_VOID MED_AEC_AfUpdateWeightUnit(
                VOS_INT16       shwM,
                VOS_INT16       shwFftLen,
                VOS_INT16       pshwFarFreq[][MED_AEC_MAX_FFT_LEN],
                VOS_INT16       pshwErrFreq[],
                VOS_INT16       pshwProp[],
                VOS_INT32       pswStep[],
                VOS_INT32       pswWeight[][MED_AEC_MAX_FFT_LEN])
{
    VOS_INT32       swCntI;
    VOS_INT32       swCntJ;
    VOS_INT32       swTmp;

    ae_p24x2s  aepF, aepE, aepP, aepS;
    ae_q56s    aeqS, aeqW, aeqSw1, aeqSw2;

    /* 4字节对齐保护 */
    if (((((VOS_INT32)&pshwFarFreq[0][0]) & 0x03) != 0)
      || ((((VOS_INT32)&pshwErrFreq[0]) & 0x03) != 0)
      || ((((VOS_INT32)&pshwProp[0]) & 0x03) != 0))
    {
        return;
    }

    for (swCntI = shwM - 1; swCntI >= 0; swCntI--)
    {
        /* 更新pswWeight[swCntI+1][0] */
        /*
        ...
        */
        swTmp = (VOS_INT32)pshwFarFreq[swCntI+1][0] * (VOS_INT32)pshwErrFreq[0];
        aepS  = *((ae_p16x2s *)&swTmp);
        aeqW  = *((ae_q32s *)&pswWeight[swCntI][0]);

        aepP  = *((ae_p16s *)&pshwProp[swCntI]);
        aeqS  = *((ae_q32s *)&pswStep[0]);
        aeqS  = AE_MULFQ32SP16S_L(aeqS, aepP);

        aeqSw1 = AE_MULQ32SP16U_H(aeqS, aepS);
        aeqSw2 = AE_MULQ32SP16S_L(aeqS, aepS);
        aeqSw2 = AE_SLLISQ56S(aeqSw2, 16);
        aeqSw2 = AE_ADDSQ56S(aeqSw1, aeqSw2);
        aeqSw2 = AE_SLLISQ56S(aeqSw2, 16);
        aeqSw2 = AE_ADDSQ56S(aeqSw2, aeqW);
        aeqSw2 = AE_SATQ48S(aeqSw2);

        pswWeight[swCntI][0] = AE_TRUNCA32Q48(aeqSw2);

        /* 更新pswWeight[swCntI+1][1] */
        /*
        ...
        pstAf->aswWeight[swCntI][1]
               = CODEC_OpL_add(pstAf->aswWeight[swCntI][1],
                             CODEC_OpFloatTo32(CODEC_OpFloatAMul32U(swTmp1,  swTmp2)));
        */
        swTmp = (VOS_INT32)pshwFarFreq[swCntI+1][1] * (VOS_INT32)pshwErrFreq[1];
        aepS  = *((ae_p16x2s *)&swTmp);
        aeqW  = *((ae_q32s *)&pswWeight[swCntI][1]);

        aepP  = *((ae_p16s *)&pshwProp[swCntI]);
        aeqS  = *((ae_q32s *)&pswStep[shwFftLen>>1]);
        aeqS  = AE_MULFQ32SP16S_L(aeqS, aepP);
        ...
        pswWeight[swCntI][1] = AE_TRUNCA32Q48(aeqSw2);

        for (swCntJ = 1; swCntJ <= ((shwFftLen >> 1) - 1); swCntJ++)
        {
            aepF  = *((ae_p16x2s *)&pshwFarFreq[swCntI + 1][2 * swCntJ]);
            aepE  = *((ae_p16x2s *)&pshwErrFreq[2 * swCntJ]);

            /* 更新pswWeight[swCntI][2 * swCntJ] */
            /*
            ...
            pstAf->aswWeight[swCntI][2 * swCntJ]
                   = CODEC_OpL_add(pstAf->aswWeight[swCntI][2 * swCntJ],
                                 CODEC_OpFloatTo32(CODEC_OpFloatAMul32U(swTmp1, swTmp2)));
            */
            aeqS   = AE_MULZAAP24S_HH_LL(aepF, aepE);
            aeqS   = AE_SATQ48S(aeqS);
            swTmp  = AE_TRUNCA32Q48(aeqS);

            aepS   = *((ae_p16x2s *)&swTmp);
            aeqW   = *((ae_q32s *)&pswWeight[swCntI][2 * swCntJ]);

            aepP   = *((ae_p16s *)&pshwProp[swCntI]);
            aeqS   = *((ae_q32s *)&pswStep[swCntJ]);
            aeqS  = AE_MULFQ32SP16S_L(aeqS, aepP);
            ...
            pswWeight[swCntI][2 * swCntJ] = AE_TRUNCA32Q48(aeqSw2);

            /* 更新pswWeight[swCntI][(2 * swCntJ) + 1] */
            /*
            ...
            pstAf->aswWeight[swCntI][(2 * swCntJ) + 1]
                   = CODEC_OpL_add(pstAf->aswWeight[swCntI][(2 * swCntJ) + 1],
                                 CODEC_OpFloatTo32(CODEC_OpFloatAMul32U(swTmp1, swTmp2)));
            */
            aeqSw1 = AE_MULZASP24S_HL_LH(aepF, aepE);
            aeqSw1 = AE_SATQ48S(aeqSw1);
            swTmp  = AE_TRUNCA32Q48(aeqSw1);

            aepS   = *((ae_p16x2s *)&swTmp);
            aeqW   = *((ae_q32s *)&pswWeight[swCntI][(2 * swCntJ) + 1]);
            ...
            pswWeight[swCntI][(2 * swCntJ) + 1] = AE_TRUNCA32Q48(aeqSw2);
...
/*****************************************************************************
 函 数 名  : MED_AEC_AfUpdateWeightDeCov
 功能描述  : 去除循环卷积部分
 输入参数  : pstAf->ashwFarFreq        -  远端频域信号     16Bit Q0
             pstAf->aswWeight          -  自适应滤波器系数 32Bit Q27
             pstAf->ashwError          -  残差频域信号     16Bit Q0
             pstAf->ashwProp           -  自适应率         16Bit Q15
             pstAf->stStepState.aswStep-  自适应步长       32Bit Q27
             pstAf->swCancelCount      -  帧计数器         32Bit Q0
             pstAf->shwFrameLen        -  帧长             16Bit Q0
             pstAf->shwM               -  MDF算法分块数    16Bit Q0
             pstAf->enFftLen           -  FFT阶数          16Bit Q0
 输出参数  : pstAf->aswWeight          -  自适应滤波器系数 32Bit Q27
 ...
 修改历史      :
  1.日    期   : 2012年4月10日
    作    者   : MED_AEC_AfUpdateWeightDeCov
    修改内容   : 新生成函数

*****************************************************************************/
VOS_VOID  MED_AEC_AfUpdateWeightDeCov(
                MED_AEC_AF_STRU        *pstAf,
                VOS_INT32               swCntI)
{
    VOS_INT32                           swCntJ;
    ...
    ae_q56s                             aeqWmax, aeqW0, aeqW1;
    ae_p24x2s                           aepWtmp;

    pswWeight  = pstAf->aswWeight[swCntI];
    shwFftLen  = pstAf->enFftLen;
    pashwWtmp  = MED_AEC_GetshwVecTmp640Ptr2();

    aeqWmax = AE_CVTQ48A32S(2);

    for (swCntJ = 0; swCntJ < (shwFftLen>>1); swCntJ++)
    {
        aeqW0 = *((ae_q32s *) &pswWeight[2*swCntJ]);
        aeqW1 = *((ae_q32s *) &pswWeight[2*swCntJ + 1]);

        aeqW0 = AE_ABSQ56(aeqW0);
        aeqW0 = AE_SATQ48S(aeqW0);
        aeqWmax = AE_MAXQ56S(aeqWmax, aeqW0);

        aeqW1 = AE_ABSQ56(aeqW1);
        aeqW1 = AE_SATQ48S(aeqW1);
        aeqWmax = AE_MAXQ56S(aeqWmax, aeqW1);
    }

    shwWeightShift = (39 - AE_NSAQ56S(aeqWmax)) - MED_AEC_AF_WEIGHT_POWER_THD;
    ...
    if (shwWeightShift > 0)
    {
        for (swCntJ = 0; swCntJ < (shwFftLen>>1); swCntJ++)
        {
            ...
            aeqW1 = *((ae_q32s *) &pswWeight[2*swCntJ + 1]);
      	    ...
      	    aeqW0 = AE_SRAAQ56(aeqW0, shwWeightShift);
		    aeqW0 = AE_ROUNDSQ32ASYM(aeqW0);

		    aeqW0 = AE_SLLISQ56S(aeqW0, 16);
            aeqW0 = AE_SATQ48S(aeqW0);

            aeqW1 = AE_SRAAQ56(aeqW1, shwWeightShift);
		    aeqW1 = AE_ROUNDSQ32ASYM(aeqW1);

		    aeqW1 = AE_SLLISQ56S(aeqW1, 16);
            aeqW1 = AE_SATQ48S(aeqW1);

            aepWtmp = AE_TRUNCP24Q48X2(aeqW0, aeqW1);

            *((ae_p16x2s *)&pashwWtmp[2*swCntJ]) = aepWtmp;
    ...
    if (shwWeightShift > 0)
    {
        for (swCntJ = 0; swCntJ < (shwFftLen>>1); swCntJ++)
        {
            aepWtmp = *((ae_p16x2s *)&pashwWtmp[2*swCntJ]);

            aeqW0 = AE_CVTQ48P24S_H(aepWtmp);
            aeqW1 = AE_CVTQ48P24S_L(aepWtmp);

            aeqW0 = AE_SRAIQ56(aeqW0, 16);
            aeqW1 = AE_SRAIQ56(aeqW1, 16);

            aeqW0 = AE_SLLASQ56S(aeqW0, shwWeightShift);
            aeqW0 = AE_SATQ48S(aeqW0);
            *((ae_q32s *) &pswWeight[2*swCntJ]) = aeqW0;

            aeqW1 = AE_SLLASQ56S(aeqW1, shwWeightShift);
            aeqW1 = AE_SATQ48S(aeqW1);
            *((ae_q32s *) &pswWeight[2*swCntJ + 1]) = aeqW1;
        }
    }
                ...
                MED_AEC_AF_STRU         *pstAf)
{
    ...
    /* 计算更新滤波器系数 */
    MED_AEC_AfUpdateWeightUnit(pstAf->shwM,
                               shwFftLen,
                               pstAf->ashwFarFreq,
                               &pstAf->ashwErrFreq[0],
                               &pstAf->ashwProp[0],
                               &pstAf->stStepState.aswStep[0],
                               pstAf->aswWeight);

    /* 去除循环卷积部分(详细原理参见AUMDF算法) */
    MED_AEC_AfUpdateWeightDeCov(pstAf, 0);
    MED_AEC_AfUpdateWeightDeCov(pstAf, CODEC_OpMod32(pstAf->swCancelCount, pstAf->shwM - 1) + 1);
                ...
                VOS_INT16  shwShift)
{
    ...
    /* 4字节对齐保护 */
    if (((((VOS_INT32)&pshwX[0]) & 0x03) != 0) || ((((VOS_INT32)&pshwY[0]) & 0x03) != 0))
    {
        return 0;
    }
    ...
    /* 要求shwFrameSize<512,否则存在溢出风险 */
    for (swCntI = 0; swCntI < shwFrameSize>>1; swCntI++)
    {
        aepX = *((ae_p16x2s *)&pshwX[2*swCntI]);
        aepY = *((ae_p16x2s *)&pshwY[2*swCntI]);

        AE_MULAAP24S_HH_LL(aeqS, aepX, aepY);
    }

    aeqS = AE_SRAAQ56(aeqS, shwShift);
    aeqS = AE_SATQ48S(aeqS);
