...
Word16 block_norm (Word16 * in, Word16 * out, Word16 length, Word16 headroom)
{
    ...
    if (max != 0)
    {
        ...
        if(scnt>=0)
        {
            CODEC_OpVecShl(&in[0], length, scnt, &out[0]);
        }
        else
        {
        	CODEC_OpVecShr(&in[0], length, -scnt, &out[0]);
        }
    }
    else
    {
        scnt = sub(16, headroom);
        CODEC_OpVecSet(&out[0], length, 0);
    }
    return (scnt);
}
...
Word16 vad2 (Word16 * farray_ptr, vadState2 * st)
{
    ...
    CODEC_OpVecSet(&data_buffer[0], DELAY, 0);
    ...
    CODEC_OpVcMultQ15Add(&input_buffer[1],
                       &input_buffer[0],
                       FRM_LEN - 1,
                       PRE_EMP_FAC,
                       &data_buffer[DELAY + 1]);
    ...
    CODEC_OpVecSet(&data_buffer[DELAY + FRM_LEN],
                 FFT_LEN - DELAY - FRM_LEN,
                 0);
    ...
    if (st->shift_state == 0)
    {
        if ( normb_shift <= (Word16)(-FFT_HEADROOM+2) )
        {
            state_change = 1;
            st->shift_state = 1;
        }
    ...
    {
        if ( normb_shift >= (Word16)(-FFT_HEADROOM+5))
        {
            state_change = 1;
            st->shift_state = 0;
        }
    ...
    for (i = LO_CHAN; i <= HI_CHAN; i++)
    {
        ...
        Lenrg = CODEC_OpVvMac(&data_buffer[2 * j1],
                            &data_buffer[2 * j1],
                            2 * (j2 - j1 + 1),
                            0);
    ...
    if ( (st->Lframe_cnt <= (Word32)4) || st->fupdate_flag == TRUE)
    {
    ...
    if ( 1 == st->Lframe_cnt)
    {
        CODEC_OpVecCpy(&st->ch_enrg_long_db[LO_CHAN],
                     &ch_enrg_db[LO_CHAN],
                     HI_CHAN-LO_CHAN+1);
    }
    else
    ...
    else if ( Ltce > (Word32)noise_floor_chan[st->shift_state] )
    {
        if ( ch_enrg_dev < (Word16)DEV_THLD )
        {
            if (p2a_flag == FALSE)
            {
                if (st->LTP_flag == FALSE)
                {
                    ...
                    if ( st->update_cnt >= (Word16)UPDATE_CNT_THLD )
                    {
                        update_flag = TRUE;
    ...
    if ( st->hyster_cnt > (Word16)HYSTER_CNT_THLD )
    {
        st->update_cnt = 0;
    }
...
int vad2_reset (vadState2 * st)
{
    ...
    if (st == (vadState2 *) AMR_NULL){
        return -1;
    }
    ptr = (Word16 *)st;

    CODEC_OpVecSet(&ptr[0],
                 sizeof(vadState2)/2,
                 0);
