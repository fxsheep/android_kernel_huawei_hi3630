...
/* cod_amrState 状态全局变量,在本模块的init函数中作为预先分配的内存替换malloc */
cod_amrState g_stAmrCodState;
...
int cod_amr_init (cod_amrState **state, Flag dtx)
{
  ...
  if (state == (cod_amrState **) AMR_NULL){
      return -1;
  }
  *state = AMR_NULL;
  ...
  s->lspSt = AMR_NULL;
  s->clLtpSt = AMR_NULL;
  s->gainQuantSt = AMR_NULL;
  s->pitchOLWghtSt = AMR_NULL;
  s->tonStabSt = AMR_NULL;
  s->vadSt = AMR_NULL;
  s->dtx_encSt = AMR_NULL;
  s->dtx = dtx;
  s->pfCallBack = AMR_NULL;
  s->pstSidState = AMR_NULL;
...
int cod_amr_reset (cod_amrState *st)
{
   ...
   /* OL LTP states */
   CODEC_OpVecSet(&st->old_lags[0], 5, 40);
...
void cod_amr_exit (cod_amrState **state)
{
   ...
   sid_sync_exit(&(*state)->pstSidState);
...
)
{
   ...
   CODEC_OpVecCpy(st->new_speech, new_speech, L_FRAME);
   ...
   *usedMode = mode;
   ...
   sid_sync(st->pstSidState, *usedMode, penTxFrmType);

   /* Call call-back function */
   if(AMR_NULL != st->pfCallBack)
   {
       ...
       switch (*penTxFrmType)
        {
            ...
            case TX_SID_BAD:
            {
                fCallBack(CODEC_AMR_FRM_TYPE_AMR_SID);
                break;
            }
            default:
            {
                fCallBack(CODEC_AMR_FRM_TYPE_NO_DATA);
            }
        }
   }
   ...
   if ( (enum Mode)MRDTX == *usedMode )
   {
      ...
      /* Reset lsp states */
      lsp_reset(st->lspSt);
      CODEC_OpVecCpy(st->lspSt->lsp_old, lsp_new, M);
      CODEC_OpVecCpy(st->lspSt->lsp_old_q, lsp_new, M);
       ...
       subfrNr++, i_subfr += L_FRAME_BY2)
   {
      ...
      if (( mode != (enum Mode)MR475 ) && ( mode != (enum Mode)MR515 ))
      {
   ...
   for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
   {
      ...
      if ((evenSubfr != 0) && ((enum Mode)MR475 == *usedMode))
      {
         CODEC_OpVecCpy(mem_syn_save, st->mem_syn, M);
         CODEC_OpVecCpy(mem_w0_save, st->mem_w0, M);
         CODEC_OpVecCpy(mem_err_save, st->mem_err, M);
         sharp_save = st->sharp;
      }
      ...
      { /* MR475 */
         ...
         if (evenSubfr != 0)
         {
             CODEC_OpVecCpy(h1_sf0, st->h1, L_SUBFR);
         }
      }

      /* copy the LP residual (res2 is modified in the CL LTP search)    */
      CODEC_OpVecCpy(res2, res, L_SUBFR);
      ...
      else
      {
         ...
         {
            i_subfr_sf0 = i_subfr;
            CODEC_OpVecCpy(xn_sf0, xn, L_SUBFR);
            CODEC_OpVecCpy(y2_sf0, y2, L_SUBFR);
            CODEC_OpVecCpy(code_sf0, code, L_SUBFR);
            T0_sf0 = T0;
            T0_frac_sf0 = T0_frac;
         ...
         else
         {
            ...
            CODEC_OpVecCpy(st->mem_err, mem_err_save, M);
...
the_end:
    ...
    *--------------------------------------------------*/
   CODEC_OpVecCpy(&st->old_wsp[0],
                &st->old_wsp[L_FRAME],
                PIT_MAX);

   CODEC_OpVecCpy(&st->old_speech[0],
                &st->old_speech[L_FRAME],
                L_TOTAL - L_FRAME);
