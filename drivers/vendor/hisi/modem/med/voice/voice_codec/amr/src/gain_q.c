...
/* gainQuantState 状态全局变量,在本模块的init函数中作为预先分配的内存替换malloc */
gainQuantState g_stAmrGainQuantState;
...
int gainQuant_init (gainQuantState **state)
{
  ...
  if (state == (gainQuantState **) AMR_NULL){
      return -1;
  }
  *state = AMR_NULL;

  /* allocate memory */
  s = &g_stAmrGainQuantState;

  s->gain_idx_ptr = AMR_NULL;

  s->gc_predSt = AMR_NULL;
  s->gc_predUnqSt = AMR_NULL;
  s->adaptSt = AMR_NULL;

  /* Init sub states */
  if (   gc_pred_enc_init(&s->gc_predSt, &s->gc_predUnqSt)
