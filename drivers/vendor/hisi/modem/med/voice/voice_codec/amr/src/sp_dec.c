...
/* Speech_Decode_FrameState 状态全局变量,在本模块的init函数中作为预先分配的内存替换malloc */
Speech_Decode_FrameState g_stAmrDecFrameState;
                              ...
                              char *id)
{
  ...
  if (state == (Speech_Decode_FrameState **) AMR_NULL){
      return -1;
  }
  *state = AMR_NULL;

  /* allocate memory */
  s = &g_stAmrDecFrameState;

  s->decoder_amrState = AMR_NULL;
  s->post_state = AMR_NULL;
  s->postHP_state = AMR_NULL;
...
int Speech_Decode_Frame_reset (Speech_Decode_FrameState *state)
{
  ...
  state->shwRxSidResetFlg = AMR_SID_RESET_FLG_NO;
...
)
{
  ...
  CODEC_OpVecSet(parm, MAX_PRM_SIZE + 1, 0);
...
#if !defined(NO13BIT)
  /* Truncate to 13 bits */
  CODEC_OpVcAnd(&synth[0], L_FRAME, (Word16)0xfff8, &synth[0]);
#endif
