...
/* vadState1 状态全局变量,在本模块的init函数中作为预先分配的内存替换malloc */
vadState1 g_stAmrVad1State;
                         ...
                         )
{
  ...
  if (st->complex_warning != 0)
  {
     ...
     if ( st->stat_count < (Word16)CAD_MIN_STAT_COUNT )
     {
  ...
  if (( 0x6000 == (st->pitch & 0x6000)) ||
      ( 0x7c00 == (st->tone & 0x7c00)))
  {
  ...
  else
  {
     ...
     else
     {
        ...
        for (i = 0; i < COMPLEN; i++)
        {
           ...
           if ( num < (Word16)STAT_THR_LEVEL)
           {
              num = STAT_THR_LEVEL;
           }

           if ( denom < (Word16)STAT_THR_LEVEL)
           {
              ...
              )
{
   ...
   if ( noise_level > (Word16)HANG_NOISE_THR )
   {
   ...
   if ( st->complex_hang_timer > (Word16)CVAD_HANG_LIMIT)
   {

      if ( st->complex_hang_count < (Word16)CVAD_HANG_LENGTH)
      {
   ...
   else
   {
      ...
      if (((st->vadreg & 0x3ff0) == 0) &&
          ( st->corr_hp_fast > (Word16)CVAD_THRESH_IN_NOISE))
      {
         return 1;
      }
   }
                  ...
                  )
{
   ...
   for (i = 0; i < COMPLEN; i++)
   {
      ...
      { /* update downwards*/
         ...
         if ( st->bckr_est[i] < (Word16)NOISE_MIN )
         {
      ...
      { /* update upwards */
         ...
         if (st->bckr_est[i] > (Word16)NOISE_MAX)
         {
   ...
   CODEC_OpVecCpy(&st->old_level[0], &level[0], COMPLEN);
         ...
         )
{
   ...
   if ( st->best_corr_hp < st->corr_hp_fast) /* decrease */
   {
      ...
      if ( st->corr_hp_fast < (Word16)CVAD_THRESH_ADAPT_HIGH)
      {  /* low state  */
   ...
   if ( st->corr_hp_fast < (Word16)CVAD_MIN_CORR )
   {
                          ...
                          )
{
   ...
   if (low_power == 0)
   {
      ...
      if (st->corr_hp_fast > (Word16)CVAD_THRESH_ADAPT_LOW )
      {
         st->complex_low = st->complex_low | 0x4000;
      }
   }

   if ( st->corr_hp_fast > (Word16)CVAD_THRESH_HANG )
   {
   ...
   return (( 0x7f80 == (st->complex_high & 0x7f80)) ||
           ( 0x7fff == (st->complex_low & 0x7fff)));
}
...
int vad1_reset (vadState1 *state)
{
   ...
   CODEC_OpVecSet(&state->a_data3[0], 5, 0);
                          ...
                          )
{
   ...
   for (i = 0; i < 2; i++)
   {
      ...
      if ( (abs_s (sub (st->oldlag, T_op[i]))) < (Word16)LTHRESH)
      {
   ...
   if (( add (st->oldlag_count, lagcount) )>= NTHRESH )
   {
           ...
           )
{
   ...
   */
   ...
   pow_sum = CODEC_OpVvMac(&in_buf[-LOOKAHEAD], &in_buf[-LOOKAHEAD], FRAME_LEN, 0);

   /*
     If input power is very low, clear pitch flag of the current frame
     */

   if ( pow_sum < (Word32)POW_PITCH_THR )
   {
   ...
   if ( pow_sum < (Word32)POW_COMPLEX_THR )
   {
