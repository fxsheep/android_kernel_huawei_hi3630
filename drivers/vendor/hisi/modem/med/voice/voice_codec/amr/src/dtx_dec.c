...
/* dtx_decState 状态全局变量,在本模块的init函数中作为预先分配的内存替换malloc */
dtx_decState g_stAmrDtxDecState;
...
int dtx_dec_reset (dtx_decState *st)
{
   ...
   CODEC_OpVecCpy(&st->lsp[0],
                lsp_init_data,
                M);

   CODEC_OpVecCpy(&st->lsp_old[0],
                lsp_init_data,
                M);
   ...
   CODEC_OpVecCpy(&st->lsf_hist[0],
                mean_lsf,
                M);

   for (i = 1; i < DTX_HIST_SIZE; i++)
   {
      CODEC_OpVecCpy(&st->lsf_hist[M*i],
                   &st->lsf_hist[0],
                   M);
   }
   ...
   CODEC_OpVecSet(&st->log_en_hist[0], DTX_HIST_SIZE, st->log_en);
   ...
   )
{
       ...
       (st->sid_frame != 0))
   {
      ...
      CODEC_OpVecCpy(&st->lsf_hist[ptr],
                   &st->lsf_hist[st->lsf_hist_ptr],
                   M);

      ptr = add(st->log_en_hist_ptr,1);

      if ( DTX_HIST_SIZE == ptr)
      {
         ptr = 0;
      }
      ...
      CODEC_OpVecSet((Word16 *)&L_lsf[0], 2 * M, 0);
      ...
      CODEC_OpVecCpy(st->lsf_hist_mean,
                   st->lsf_hist,
                   80);
      ...
      for (i = 0; i < M; i++)
      {
         ...
         for (j = 0; j < 8; j++)
         {
            ...
            if ( st->lsf_hist_mean[i+j*M] > (Word16)655 )
            {
               st->lsf_hist_mean[i+j*M] =
            ...
            if ( st->lsf_hist_mean[i+j*M] > (Word16)1310 )
            {
   ...
   if (st->sid_frame != 0 )
   {
      ...
      CODEC_OpVecCpy(st->lsp_old, st->lsp, M);
      ...
      if (st->valid_data != 0 )  /* new data available (no CRC) */
      {
         ...
         if ( tmp_int_length > (Word16)32 )
         {
            tmp_int_length = 32;
         }

         if (tmp_int_length >= (Word16)2 )
         {
   ...
   if ( lsf_variab_factor > (Word16)4096 )
   {
   ...
   CODEC_OpVecCpy(lsf_int_variab, lsf_int, M);
   ...
   CODEC_OpVecCpy(lsfState->past_lsf_q, lsf_int, M);
   ...
   CODEC_OpVecCpy(&A_t[0],           acoeff, M + 1);
   CODEC_OpVecCpy(&A_t[M + 1],       acoeff, M + 1);
   CODEC_OpVecCpy(&A_t[2 * (M + 1)], acoeff, M + 1);
   CODEC_OpVecCpy(&A_t[3 * (M + 1)], acoeff, M + 1);
                             ...
                             Word16 frame[])
{
   ...
   if (80 == st->lsf_hist_ptr)
   {
      st->lsf_hist_ptr = 0;
   }

   CODEC_OpVecCpy(&st->lsf_hist[st->lsf_hist_ptr],
                &lsf[0],
                M);

   /* compute log energy based on frame energy */
   L_frame_en = CODEC_OpVvMac(&frame[0],
                            &frame[0],
                            L_FRAME,
                            0); /* Q0 */
   ...
   if ( DTX_HIST_SIZE == st->log_en_hist_ptr)
   {
      st->log_en_hist_ptr = 0;
   }
                      ...
                      )
{
   ...
   if (( RX_SID_FIRST == frame_type)   ||
       ( RX_SID_UPDATE == frame_type)  ||
       ( RX_SID_BAD == frame_type)     ||
       (((DTX == st->dtxGlobalState) ||
         (DTX_MUTE == st->dtxGlobalState)) &&
        ((RX_NO_DATA == frame_type) ||
         (RX_SPEECH_BAD == frame_type) ||
         (RX_ONSET == frame_type))))
   {
      newState = DTX;
      ...
      if ((DTX_MUTE == st->dtxGlobalState) &&
          ((RX_SID_BAD == frame_type) ||
           (RX_SID_FIRST ==  frame_type) ||
           (RX_ONSET ==  frame_type) ||
           (RX_NO_DATA == frame_type)))
      {
         newState = DTX_MUTE;
   ...
   */
   ...
   if ( newState != SPEECH )
   {
      ...
      else if ( RX_SID_UPDATE == frame_type )
      {
         st->sid_frame = 1;
         st->valid_data = 1;
      }
      else if ( RX_SID_BAD == frame_type )
      {
         st->sid_frame = 1;
