...
{
    CODEC_OpVecSet(mem, 18, 0);                     /* old_A[0..M-1] = 0, old_rc[0..1] = 0 */
    return;
}
...
)
{
    ...
    Word16 Ah[AMRWB_M + 1], Al[AMRWB_M + 1];           /* LPC coef. in double prec.                   */
    Word16 Anh[AMRWB_M + 1], Anl[AMRWB_M + 1];         /* LPC coef.for next iteration in double prec. */
    Word32 i, j, t0, t1, t2;                     /* temporary variable                          */
    Word16 *old_A, *old_rc;
    ...
    for (i = 2; i <= AMRWB_M; i++)
    {
        ...
        if (sub(abs_s(Kh), 32750) > 0)
        {
            ...
            CODEC_OpVecCpy(&A[1], old_A, AMRWB_M);
