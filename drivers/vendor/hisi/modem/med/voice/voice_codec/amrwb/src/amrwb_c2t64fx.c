...
)
{
    ...
    Word16 sign[AMRWB_L_SUBFR], vec[AMRWB_L_SUBFR], dn2[AMRWB_L_SUBFR];
    Word16 h_buf[4 * AMRWB_L_SUBFR];
    ...
    for (i0 = 0; i0 < AMRWB_L_SUBFR; i0 += STEP)
    {
        ps1 = dn[i0];
        alp1 = (*p0++);

        pos = -1;
        for (i1 = 1; i1 < AMRWB_L_SUBFR; i1 += STEP)
        {
            ps2 = add(ps1, dn[i1]);
