...
XT_INLINE Word16 Interpol(                    /* return result of interpolation */
...
)
{
    ...
    ae_p24x2s   aep_x1, aep_x2, aep_fir1, aep_fir2, aep_tmp;
    ae_q56s     aeq_L_sum;
    ...
    for (i = 0, k = resol - 1 - frac; i < nb_coef; i++)
    {
        /* L_sum = L_mac(L_sum, x[2*i],   fir[k]);*/
        aep_x1   = *((ae_p16s *)&x[2*i]);
        aep_fir1 = *((ae_p16s *)&fir[k]);
        k += resol;
        AE_MULAFS32P16S_LL(aeq_L_sum, aep_x1, aep_fir1);

        /* L_sum = L_mac(L_sum, x[2*i+1], fir[k]);*/

        aep_x2   = *((ae_p16s *)&x[2*i+1]);
        aep_fir2 = *((ae_p16s *)&fir[k]);
        k += resol;
        AE_MULAFS32P16S_LL(aeq_L_sum, aep_x2, aep_fir2);
    }

    aeq_L_sum = AE_SLLASQ56S(aeq_L_sum, 1);
    aeq_L_sum = AE_SATQ48S(aeq_L_sum);

    aep_tmp = AE_ROUNDSP16Q48ASYM(aeq_L_sum);

    return AE_TRUNCA16P24S_L(aep_tmp);
}
...
{
    CODEC_OpVecSet(mem, 2 * NB_COEF_DOWN, 0);
    return;
}
...
{
    Word16 lg_down;
    Word32 signal_32[(L_FRAME16k + (2 * NB_COEF_DOWN))/2];
    Word16 *signal = (Word16*)signal_32;

    CODEC_OpVecCpy(signal, mem, 2 * NB_COEF_DOWN);

    CODEC_OpVecCpy(signal + (2 * NB_COEF_DOWN), sig16k, lg);

    lg_down = mult(lg, DOWN_FAC);

    Down_samp(signal + NB_COEF_DOWN, sig12k8, lg_down);

    CODEC_OpVecCpy(mem, signal + lg, 2 * NB_COEF_DOWN);
...
{
    CODEC_OpVecSet(mem, 2 * NB_COEF_UP, 0);
    return;
}
...
{
    Word16 lg_up;
    Word16 signal[AMRWB_L_SUBFR + (2 * NB_COEF_UP)];

    CODEC_OpVecCpy(signal, mem, 2 * NB_COEF_UP);

    CODEC_OpVecCpy(signal + (2 * NB_COEF_UP), sig12k8, lg);

    lg_up = shl(mult(lg, UP_FAC), 1);

    Up_samp(signal + NB_COEF_UP, sig16k, lg_up);

    CODEC_OpVecCpy(mem, signal + lg, 2 * NB_COEF_UP);
