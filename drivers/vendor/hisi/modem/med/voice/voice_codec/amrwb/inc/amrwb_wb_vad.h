...
typedef struct
{
    Word16 bckr_est[AMRWB_COMPLEN];              /* background noise estimate                */
    Word16 ave_level[AMRWB_COMPLEN];             /* averaged input components for stationary */
                                           /* estimation                               */
    Word16 old_level[AMRWB_COMPLEN];             /* input levels of the previous frame       */
    Word16 sub_level[AMRWB_COMPLEN];             /* input levels calculated at the end of a frame (lookahead)  */
...
Word16 AMRWB_vad_init(VadVars ** st);
Word16 AMRWB_vad_reset(VadVars * st);
void AMRWB_vad_exit(VadVars ** st);
void AMRWB_vad_tone_detection(VadVars * st, Word16 p_gain);
Word16 AMRWB_vad(VadVars * st, Word16 in_buf[]);
