...
)
{
    ...
    CODEC_OpVecSet(mem, PRED_ORDER, -14336);
...
)
{
     ...
     *  coeff[4] =  2 y1 y2                                             *
     *                                                                  *
     * Product <y1 y1> and <xn y1> have been compute in AMRWB_G_pitch() and   *
    ...
    gcode0 = extract_l(AMRWB_Pow2(14, frac));    /* Put 14 as exponent so that  */
    ...
    e_max = CODEC_OpVecMax(exp_max, 5, 0);
    ...
    for (i = 0; i < size; i++)
    {
        ...
        if (L_tmp < dist_min)/*(L_sub(L_tmp, dist_min) < (Word32) 0)*/
        {
            dist_min = L_tmp;
            index = i;
        }
