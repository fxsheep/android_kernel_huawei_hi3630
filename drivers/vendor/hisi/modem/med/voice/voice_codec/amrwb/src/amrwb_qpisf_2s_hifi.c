...
)
{
    ...
    Word32 isf32[ORDER/2];
    Word16 *isf = (Word16*)isf32;
    Word32 isf_stage232[ORDER/2];
    Word16 *isf_stage2 = (Word16*)isf_stage232;
    ...
    VQ_stage1(&isf[0], dico1_isf, 9, SIZE_BK1, surv1, 4);
    ...
    for (k = 0; k < 4; k++)
    {
        /*for (i = 0; i < 9; i++)
        {
            isf_stage2[i] = sub(isf[i], dico1_isf[i + surv1[k] * 9]);
        }*/
        CODEC_OpVvSub(isf, &dico1_isf[surv1[k] * 9], 9, isf_stage2);

        tmp_ind[0] = Sub_VQ_dim3(&isf_stage2[0], dico21_isf, SIZE_BK21, &min_err);
        temp = min_err;
        tmp_ind[1] = Sub_VQ_dim3(&isf_stage2[3], dico22_isf, SIZE_BK22, &min_err);
        temp = L_add(temp, min_err);
        tmp_ind[2] = Sub_VQ_dim3(&isf_stage2[6], dico23_isf, SIZE_BK23, &min_err);
        temp = L_add(temp, min_err);
    ...
    VQ_stage1(&isf[9], dico2_isf, 7, SIZE_BK2, surv1, 4);
    ...
    for (k = 0; k < 4; k++)
    {
        /*for (i = 0; i < 7; i++)
        {
            isf_stage2[i] = sub(isf[9 + i], dico2_isf[i + surv1[k] * 7]);
        }*/
        CODEC_OpVvSub(&isf[9], &dico2_isf[surv1[k] * 7], 7, isf_stage2);
...
)
{
    ...
    if (bfi == 0)                          /* Good frame */
    {
        ...
        CODEC_OpVecCpy(isf_q, &dico1_isf[indice[0] * 9], 9);

        /*for (i = 0; i < 7; i++)
        {
            isf_q[i + 9] = dico2_isf[indice[1] * 7 + i];
        }*/
        CODEC_OpVecCpy(&isf_q[9], &dico2_isf[indice[1] * 7], 7);
        ...
        CODEC_OpVvAdd(isf_q, &dico21_isf[indice[2] * 3], 3, isf_q);

        /*for (i = 0; i < 3; i++)
        {
            isf_q[i + 3] = add(isf_q[i + 3], dico22_isf[indice[3] * 3 + i]);
        }*/
        CODEC_OpVvAdd(&isf_q[3], &dico22_isf[indice[3] * 3], 3, &isf_q[3]);

        /*for (i = 0; i < 3; i++)
        {
            isf_q[i + 6] = add(isf_q[i + 6], dico23_isf[indice[4] * 3 + i]);
        }*/
        CODEC_OpVvAdd(&isf_q[6], &dico23_isf[indice[4] * 3], 3, &isf_q[6]);

        /*for (i = 0; i < 3; i++)
        {
            isf_q[i + 9] = add(isf_q[i + 9], dico24_isf[indice[5] * 3 + i]);
        }*/
        CODEC_OpVvAdd(&isf_q[9], &dico24_isf[indice[5] * 3], 3, &isf_q[9]);

        /*for (i = 0; i < 4; i++)
        {
            isf_q[i + 12] = add(isf_q[i + 12], dico25_isf[indice[6] * 4 + i]);
        }*/
        CODEC_OpVvAdd(&isf_q[12], &dico25_isf[indice[6] * 4], 4, &isf_q[12]);
        ...
        if (enc_dec)
        {
            for (i = 0; i < AMRWB_M; i++)
            {
                ...
                {
                    isf_buf[j * AMRWB_M + i] = isf_buf[(j - 1) * AMRWB_M + i];
                }
                isf_buf[i] = isf_q[i];
            }
        }
    ...
    {                                      /* bad frame */
        for (i = 0; i < AMRWB_M; i++)
        {
            ...
            {
                L_tmp = L_mac(L_tmp, isf_buf[j * AMRWB_M + i], 8192);
            }
            ref_isf[i] = round(L_tmp);
        }
        ...
        CODEC_OpVvFormWithCoef(isfold, ref_isf, ORDER, ALPHA, ONE_ALPHA, isf_q);
...
)
{
    ...
    if (bfi == 0)                          /* Good frame */
    {
        ...
        CODEC_OpVvAdd(&isf_q[0], &dico21_isf_36b[indice[2] * 5], 5, &isf_q[0]);

        /*for (i = 0; i < 4; i++)
        {
            isf_q[i + 5] = add(isf_q[i + 5], dico22_isf_36b[indice[3] * 4 + i]);
        }*/
        CODEC_OpVvAdd(&isf_q[5], &dico22_isf_36b[indice[3] * 4], 4, &isf_q[5]);

        /*for (i = 0; i < 7; i++)
        {
            isf_q[i + 9] = add(isf_q[i + 9], dico23_isf_36b[indice[4] * 7 + i]);
        }*/
        CODEC_OpVvAdd(&isf_q[9], &dico23_isf_36b[indice[4] * 7], 7, &isf_q[9]);
...
)
{
    ...
    for (i = 0; i < n - 1; i++)
    {
        if (isf[i] < isf_min)/*(sub(isf[i], isf_min) < 0)*/
        {
            isf[i] = isf_min;
        }
...
)
{
    ...
    ae_p24x2s aep_x, aep_dico, aep_i, aep_index;
    ae_q56s   aeq_dist, aeq_min;
    xtbool b;

    dist_min = MAX_32;
    aeq_min  = AE_CVTQ48A32S(MAX_32);
    p_dico = dico;

    index = 0;
    aep_index = AE_ZEROP48();
    ...
    for (i = 0; i < dico_size; i++)
    {
        ...
        for (j = 0; j < dim; j++)
        {
            ...
            aep_x    = *((ae_p16s *)&x[j]);
            aep_dico = *((ae_p16s *)p_dico++);
            aep_x    = AE_SUBSP24S(aep_x, aep_dico);

            AE_MULAFS32P16S_LL(aeq_dist, aep_x, aep_x);
        }
        ...
        */
        ...
        AE_MINBQ56S(aeq_min, aeq_dist, aeq_min, b);
        AE_MOVTP48(aep_index, aep_i, b);
    }

    // *distance = dist_min;
    *((ae_q32s *) distance) = aeq_min;

    /* Reading the selected vector */
    index = AE_TRUNCA16P24S_H(aep_index);
...
{
    Word16 i, index, temp, *p_dico;
    Word32 dist_min;

    ae_p24x2s aep_x1, aep_x2, aep_x3, aep_dico1, aep_dico2, aep_dico3, aep_i, aep_index;
    ae_q56s   aeq_dist, aeq_min;
    xtbool b;
    ...
    for (i = 0; i < dico_size; i++)
    {
        ...
        aep_dico1 = *((ae_p16s *)p_dico++);
        aep_dico1 = AE_SUBSP24S(aep_x1, aep_dico1);

        AE_MULAFS32P16S_LL(aeq_dist, aep_dico1, aep_dico1);
        aep_dico2 = *((ae_p16s *)p_dico++);

        aep_dico2 = AE_SUBSP24S(aep_x2, aep_dico2);
        AE_MULAFS32P16S_LL(aeq_dist, aep_dico2, aep_dico2);

        aep_dico3 = *((ae_p16s *)p_dico++);
        aep_dico3    = AE_SUBSP24S(aep_x3, aep_dico3);

        AE_MULAFS32P16S_LL(aeq_dist, aep_dico3, aep_dico3);
...
{
    Word32 dist[SIZE_BK];
    Word16 i, j, k, *p_dico0, *p_dico1, *p_dico2, *p_dico3;
    ae_p24x2s aep_temp0, aep_temp1, aep_temp2, aep_temp3, aep_x, aep_k, aep_id;
    ae_q56s   aeq_dist0, aeq_dist1, aeq_dist2, aeq_dist3, aeq_min, aeq_dist;
    xtbool    ae_flag;
    ...
    for (i = 0; i < SIZE_BK; i += 4)
    {
        aeq_dist0 = AE_ZEROQ56();
        aeq_dist1 = AE_ZEROQ56();
        aeq_dist2 = AE_ZEROQ56();
        aeq_dist3 = AE_ZEROQ56();

        aep_x = *((ae_p16s *)x);

        for (j = 0; j < dim; j++)
        {
            aep_temp0 = *((ae_p16s *)p_dico0++);
            aep_temp0 = AE_SUBSP24S(aep_x, aep_temp0);
            AE_MULAFS32P16S_LL(aeq_dist0, aep_temp0, aep_temp0);

            aep_temp1 = *((ae_p16s *)p_dico1++);
            aep_temp1 = AE_SUBSP24S(aep_x, aep_temp1);
            AE_MULAFS32P16S_LL(aeq_dist1, aep_temp1, aep_temp1);

            aep_temp2 = *((ae_p16s *)p_dico2++);
            aep_temp2 = AE_SUBSP24S(aep_x, aep_temp2);
            AE_MULAFS32P16S_LL(aeq_dist2, aep_temp2, aep_temp2);

            aep_temp3 = *((ae_p16s *)p_dico3++);
            aep_temp3 = AE_SUBSP24S(aep_x, aep_temp3);
            AE_MULAFS32P16S_LL(aeq_dist3, aep_temp3, aep_temp3);

            aep_x = *((ae_p16s *)&x[j + 1]);
        }

        p_dico0 = p_dico3;
        *((ae_q32s *)&dist[i])     = aeq_dist0;

        p_dico1 = p_dico0 + dim;
        *((ae_q32s *)&dist[i + 1]) = aeq_dist1;

        p_dico2 = p_dico1 + dim;
        *((ae_q32s *)&dist[i + 2]) = aeq_dist2;

        p_dico3 = p_dico2 + dim;
        *((ae_q32s *)&dist[i + 3]) = aeq_dist3;
    ...
    for(i = 0; i < N_SURV; i++)
    {
        ...
        aeq_dist = *((ae_q32s *) dist);
        for(k = 0; k < SIZE_BK; k++ )
        {
            aep_k    = AE_CVTP24A16(k);

            AE_MINBQ56S(aeq_min, aeq_dist, aeq_min, ae_flag);
            AE_MOVTP48(aep_id, aep_k, ae_flag);

            aeq_dist = *((ae_q32s *) &dist[k + 1]);
        }

        *((ae_p16x2s *)&index[i]) = aep_id;
        dist[index[i]] = MAX_32;
    }
