...
Word32 AMRWB_Pow2(                               /* (o) Q0  : result       (range: 0<=val<=0x7fffffff) */
...
)
{
    ...
    addr = ((Word32)x|(Word32)y) & 0x3 ;

    /*for (i = 0; i < lg; i++)
        L_sum = L_mac(L_sum, x[i], y[i]);*/
    if(0==addr)
    {
        L_sum = CODEC_OpVvMacAlignedQuan(x, y, lg, L_sum);
    }
    else
    {
        L_sum = CODEC_OpVvMac(x, y, lg, L_sum);
    }
