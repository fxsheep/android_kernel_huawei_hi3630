...
)
{
    ...
    Word32 x32[4];
    Word16 *x16 = (Word16 *)x32;

    if (0 != (((Word32)x) & 0x03))
    {
        CODEC_OpVecCpy(x16, x, dim);
    }
    else
    {
        x16 = x;
    ...
    for (i = 0; i < dico_size; i+=2)
    {
        ...
        dist = AMRWB_OpVvMacSub(x16, p_dico, dim, 0);
        p_dico += dim;

        if (dist < dist_min)/*(L_sub(dist, dist_min) < (Word32) 0)*/
        {
            dist_min = dist;
            index = i;
        }
...
{
    return Sub_VQ(x, dico, 3, dico_size, distance);
}
...
{
    Word16 *p_dico;
    Word32 i, l, dist_min[N_SURV_MAX], dist;
    ...
    for (i = 0; i < 256; i++)
    {
        ...
        dist = AMRWB_OpVvMacSub(x, p_dico, dim, dist);
        p_dico += dim;

        if (dist < dist_min[0])/*(L_sub(dist, dist_min[k]) < (Word32) 0)*/
        {
            for (l = (4 - 1); l > 0; l--)
            {
        ...
        else if (dist < dist_min[2])
        {
            ...
            dist_min[3] = dist_min[2];
            index[3] = index[2];

            dist_min[2] = dist;
            index[2] = i;
        }
        else if (dist < dist_min[3])
        {
            dist_min[3] = dist;
            index[3] = i;
        }
