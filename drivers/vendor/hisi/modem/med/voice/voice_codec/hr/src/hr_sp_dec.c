...
/*_________________________________________________________________________
 ...
 |_________________________________________________________________________|
*/
  /* ppsrPVecIntFilt[][]转置 用于性能优化 */
  static Shortword ppsrPVecIntFiltT[OS_FCTR][PVECINTFILT_MACS] =
  {
      {0,   -368, 762,  -1186, 1515,  31144, 1515,  -1186, 762,  -368},
      {183, -681, 1595, -3195, 7239,  29814, -2696, 611,   -38,  -67},
      {249, -897, 2214, -4898, 13835, 26040, -5089, 1842,  -622, 149},
      {251, -904, 2360, -5708, 20424, 20424, -5708, 2360,  -904, 251},
      {149, -622, 1842, -5089, 26040, 13835, -4898, 2214,  -897, 249},
      {-67, -38,  611,  -2696, 29814, 7239, -3195,  1595,  -681, 183}
  };
...
*/
  ...
  short int siUnstableFlt;                            /* Loop control variables */
...
*/
  ...
  while (siSampsSoFar < S_LEN)
  {
    ...
    if (swRemain == 0)
    {
      ...
      /*----------------------------------------*/
      CODEC_OpVecCpy(&pswLTPState[siSampsSoFar],
    ...
    else
    {
      ...
      for (i = siSampsSoFar; i < siSampsSoFar + siSampsThisPass; i++)
      {
        ...
        /*--------------------------------*/
          L_Temp = CODEC_OpVvMac(&pswLTPState[i - swIntLag - P_INT_MACS / 2],
                               &ppsrPVecIntFiltT[swRemain][0],
                                P_INT_MACS,
                               (long)32768);
...
*/
  ...
  else
  {
    ...
    L_sum = CODEC_OpVecScaleMac(&pswIn[0],
                              S_LEN,
                              0,
                              swInputRShft);
  }
...
*/
  ...
  for (siSmp = 1; siSmp < S_LEN; siSmp++)
  {
    ...
    /* sum past outputs */
    /* ---------------- */
    swLen = siSmp < NP ? siSmp : NP;

    L_Sum = CODEC_OpVvMacD(&pswCoef[0],
                         &pswInput[siSmp - 1],
                         swLen,
                         LPC_ROUND);
    ...
    if (siSmp < NP)
    {
        L_Sum = CODEC_OpVvMac(&pswState[0],
                            &pswCoef[siSmp],
                             NP-siSmp,
                             L_Sum);
    }
...
*/
  ...
  /* ---------------------- */
  /* 0th coef, with rounding */
  L_Sum = CODEC_OpVvMsu(&pswState[0],
                      &pswCoef[0],
                      NP,
                      LPC_ROUND);
  ...
  for (siSmp = 1; siSmp < S_LEN; siSmp++)
  {
    ...
    L_Sum = CODEC_OpVvMsuD(&pswCoef[0],
                         &pswFiltOut[siSmp - 1],
                         swLen,
                         LPC_ROUND);
...
*/
  ...
  if (swUvCode == 0)
  {
    ...
    CODEC_OpVecCpy(&pswPPreState[0],
                 &pswPPreState[S_LEN],
                 LTP_LEN - S_LEN);

    CODEC_OpVecCpy(&pswPPreState[LTP_LEN - S_LEN],
                 &pswExcite[0],
                 S_LEN);

    if (pswExciteOut != pswExcite)
    {
        CODEC_OpVecCpy(&pswExciteOut[0],
                     &pswExcite[0],
                     S_LEN);
    }
  }
  else
  {
    ...
    else
    {
      ...
      L_OrigEnergy = CODEC_OpVvMac(pswExcite,
                                 pswExcite,
                                 S_LEN,
                                 0);
    ...
    for (i = 0; i < S_LEN; i++)
    {
      ...
      L_1 = CODEC_OpVvMac(&pswPPreCurr[i - swIntLag - P_INT_MACS / 2],
                        pswInterpCoefs,
                        P_INT_MACS - 1,
                        L_1);
...
*/
  ...
  if (!rcToADp(ASCALE, pswFrmKs, pswFrmAs))
  {
    ...
    CODEC_OpVvMultR(&pswFrmAs[0],
                  &psrSPFDenomWidenCf[0],
                  NP,
                  &pswFrmPFDenom[0]);
  ...
  for (giSfrmCnt = 0; giSfrmCnt < 4; giSfrmCnt++)
  {
    ...
    CODEC_OpVecCpy(&pswLtpStateBaseDec[0],
                 &pswLtpStateBaseDec[S_LEN],
                 LTP_LEN);
