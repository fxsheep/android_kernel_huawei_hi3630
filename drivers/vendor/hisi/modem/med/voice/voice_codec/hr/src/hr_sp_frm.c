...
/* ppsrCGIntFilt[]转置数组ppsrCGIntFiltT声明 */
extern ShortwordRom ppsrCGIntFiltT[OS_FCTR][CGINTFILT_MACS];
...
#define HR_STEP5( j )       \
    ...
    switch (swSegmentOrder)
    {
        case 3:  HR_STEP1(0, 0);  HR_STEP1(0, 1); HR_STEP2();
                 HR_STEP3(0, -1); HR_STEP4(0, 0); HR_STEP4(0, 1);
                 HR_STEP5(0);
                 HR_STEP1(1, 0);  HR_STEP2();
                 HR_STEP4(1, 0);
                 break;
        case 4:  HR_STEP1(0, 0);  HR_STEP1(0, 1);  HR_STEP1(0, 2); HR_STEP2();
                 HR_STEP3(0, -2); HR_STEP3(0, -1); HR_STEP4(0, 0); HR_STEP4(0, 1); HR_STEP4(0, 2);
                 HR_STEP5(0);
                 HR_STEP1(1, 0);  HR_STEP1(1, 1); HR_STEP2();
                 HR_STEP3(1, -1); HR_STEP4(1, 0); HR_STEP4(1, 1);
                 HR_STEP5(1);
                 HR_STEP1(2, 0);  HR_STEP2();
                 HR_STEP4(2, 0);
                 break;
        default: break;
    }
...
*/
  ...
  for (swBig = 0; swBig < siNum; swBig++)
  {
    ...
    L_Temp1 = CODEC_OpVvMac(&pswCIn[swLagInt - swLoIntLag],
                          &ppsrCGIntFiltT[swLagRem][0],
                           CG_INT_MACS,
                          (Longword)32768);

    L_Temp2 = CODEC_OpVvMac(&pswGIn[swLagInt - swLoIntLag],
                          &ppsrCGIntFiltT[swLagRem][0],
                           CG_INT_MACS,
                          (Longword)32768);
...
*/
  ...
  for (i = 0; i < N_SUB; i++)
  {
    ...
    {
      L_ResidualEng = CODEC_OpVvMsu(&pswResidual[0],
                                  &pswResidual[0],
                                  S_LEN,
                                  L_ResidualEng);
    }
...
*/
  ...
  L_Pwr = CODEC_OpVvMac(&pswInScale[NP],
                      &pswInScale[NP],
                      F_LEN,
                      0);
...
*/
  ...
  for (k = 1; k <= NP; k++)
  {
    ...
    L_Temp = CODEC_OpVvMac(&pswVecTmp[0],
                         &pswVecTmp[0 + k],
                         S_LEN - k,
                         0);
...
*/
  ...
  if (siShift > 0)
  {
      CODEC_OpVecShl(&pswWSpeech[-LSMAX],
                    W_F_BUFF_LEN,
                    siShift,
                   &pswScaledWSpeech[-LSMAX]);
  }
  else
  {
      CODEC_OpVecShr(&pswWSpeech[-LSMAX],
                    W_F_BUFF_LEN,
                    -siShift,
                   &pswScaledWSpeech[-LSMAX]);
  }
  ...
  L_G = CODEC_OpVvMac(&pswScaledWSpeech[-LSMAX],
                    &pswScaledWSpeech[-LSMAX],
                    S_LEN,
                    0);
  ...
  L_WSfrmEng = CODEC_OpVvMac(&pswScaledWSpeech[F_LEN - S_LEN],
                           &pswScaledWSpeech[F_LEN - S_LEN],
                           S_LEN,
                           0);
  ...
  for (i = 0; i < N_SUB; i++)
  {
    ...
    {
      L_C = CODEC_OpVvMac(&pswScaledWSpeech[i * S_LEN],
                        &pswScaledWSpeech[i * S_LEN - j],
                         S_LEN,
                         0);
  ...
  L_Voicing = CODEC_OpVcMac(&pswSfrmEng[0],
                          N_SUB,
                          UV_SCALE0,
                          0);
