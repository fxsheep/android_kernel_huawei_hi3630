...
*/
  ...
  if (swVadFrmCnt)
  {
    ...
    if ((swShortBurst == 1) && (swVadFrmCnt < OVERHANG))
    {
      ...
      CODEC_OpVecSet(&pswCNGsp0Code[0],
                   N_SUB,
                   swRefGsIndex);
...
*/
  ...
  CODEC_OpVecCpy((Shortword *)&ppL_CorrHistory[siUpdPointer][0],
               (Shortword *)&pL_UnqntzdCorr[0],
               2 * (NP + 1));
...
*/
  ...
  else if ((swDecoMode == CNICONT) && (swFrameType == VALIDSID))
  {
    ...
    CODEC_OpVecCpy((Shortword *)&pL_OldCorrSeq[0],
                 (Shortword *)&pL_CorrSeq[0],
                 2 * (NP + 1));
