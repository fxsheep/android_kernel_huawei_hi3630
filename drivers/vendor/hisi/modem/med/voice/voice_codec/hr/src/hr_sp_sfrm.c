...
/*****************************************************************************
 函 数 名  : HrVecMsuMac
 功能描述  : z[i]=(sw - x[i]*cx + y[i]*cy)>>16
 输入参数  : Word16    *pshwX       -- x[i]向量指针
             Word16    *pshwY       -- y[i]向量指针
             Word16     pshwCx      -- cx
             Word16     pshwCy      -- cy
             Word32     swLen       -- 向量长度
             Word32     swInit      -- sw初始值
             Word16    *pshwZ       -- z[i]向量指针
 输出参数  : 无
 返 回 值  : static inline
 调用函数  :
 被调函数  :
...
{
    Word32    k = 0;
    ae_p24x2s aepX1, aepY1, aepZ1, aepCx, aepCy;
    ae_q56s   aeqS1, aeqS2;
    ...
    /* shwConst-->aepC.HL */
    aepCx = AE_CVTP24A16(pshwCx);
    aepCy = AE_CVTP24A16(pshwCy);
    ...
    if (CODEC_OpCheck4ByteAligned(pshwX) && CODEC_OpCheck4ByteAligned(pshwY))
    {
        ...
        for (k = 0; k < swLen>>1; k++)
        {
            ...
            aepY1 = *((ae_p16x2s *)&pshwY[2*k]);

            aeqS1 = AE_CVTQ48A32S(swInit);
            aeqS2 = AE_CVTQ48A32S(swInit);

            AE_MULSFS32P16S_LL(aeqS1, aepX1, aepCx);
            AE_MULAFS32P16S_LL(aeqS1, aepY1, aepCy);
            AE_MULSFS32P16S_HH(aeqS2, aepX1, aepCx);
            AE_MULAFS32P16S_HH(aeqS2, aepY1, aepCy);

            aepZ1 = AE_TRUNCP24Q48X2(aeqS2, aeqS1);

            *((ae_p16x2s *)&pshwZ[2*k]) = aepZ1;
        }
    }
    else
    {
        ...
        for (k = 0; k < swLen; k++)
        {
            ...
            aepZ1 = AE_TRUNCP24Q48X2(aeqS1, aeqS1);
...
*/
  ...
  *pswLtpShift = 0;                    /* Energy in weighted ltp vector =
  ...
  for (i = 0; i < iNumLags; i++)
  {
    ...
    /* ------------------------------------ */
    L_Energy = CODEC_OpVvMac(&ppswTVect[i][0],
                           &ppswTVect[i][0],
                           S_LEN,
                           0);
    ...
    /* ---------------------------------------------------------------------
     * */
    L_CrossCorr = CODEC_OpVvMac(&ppswTVect[i][0],
                              &pswPVect[0],
                              S_LEN,
                              0);
...
*/
  ...
  Shortword swNorm_energy,
         swTemp;
#ifndef _MED_C89_
  Shortword pswGivenVectTmp[S_LEN];
#endif
...
#else
    CODEC_OpVecShl(&pswGivenVect[0], S_LEN, swQShift, &pswGivenVectTmp[0]);

    HrVecMsuMac(&pswVects[iLoopCnt * S_LEN],
                &pswGivenVectTmp[0],
                SW_MIN,
                swTemp,
                S_LEN,
                0x00008000L,
                &pswVects[iLoopCnt * S_LEN]);
#endif
  }
}
...
*/
  ...
  {                                                          /* DTX mode */
    ...
    /* less than 1.0                                                       */
    /*---------------------------------------------------------------------*/
    CODEC_OpVecShr(&pswWSVec[0],
                  S_LEN,
                  2,
                 &swSampleATmp[0]);

    L_PnEnergy = CODEC_OpVvMac(&swSampleATmp[0],
                             &swSampleATmp[0],
                              S_LEN,
                              0x001dff4cL);
    ...
    if (swVoicingMode > 0)
    {
      ...
      if (siHnwNum > 0)
      {
        hnwFilt(pswWPVec, pswWPVec, 0, pswHNWCoefs, siHnwOffset,
                0, siHnwNum);
      }

      CODEC_OpVecShr(&pswPVec[0],
                   S_LEN,
                   swLtpShift,
                   &pswPVec[0]);

      CODEC_OpVecShr(&pswWPVec[0],
                   S_LEN,
                   swLtpShift,
                   &pswWPVec[0]);
    ...
    for (i = 0; i < siNumBasisVecs; i++)
    {
      ...
      if (siHnwNum > 0)
      {
        ...
        hnwFilt(&pswWBasisVecs[i * S_LEN], &pswWBasisVecs[i * S_LEN],
                0, pswHNWCoefs, siHnwOffset, 0, siHnwNum);
      }
    }
    ...
    *psiVSCode1 = v_srch(pswWSVec, pswWBasisVecs, siNumBasisVecs);
    ...
    if (siHnwNum > 0)
    {
      hnwFilt(ppswWVselpEx[0], ppswWVselpEx[0], 0, pswHNWCoefs,
  ...
  CODEC_OpVecCpy(&pswLtpStateOut[-LTP_LEN],
               &pswLtpStateOut[S_LEN - LTP_LEN],
               LTP_LEN - S_LEN);

  CODEC_OpVecCpy(&pswLtpStateOut[-S_LEN],
               &pswTempVec[0],
               S_LEN);
...
*/
  ...
  for (siI = 0; siI < siNumBasis; siI++)
  {
    ...
    L_R = CODEC_OpVvMac(&pswWBasisVecs[siI * S_LEN],
                      &pswWInput[0],
                      S_LEN,
                      0);
  ...
  for (siI = 0; siI < siNumBasis - 1; siI++)
  {
    ...
    for (siJ = siI + 1; siJ < siNumBasis; siJ++)
    {
      ...
      L_D = CODEC_OpVvMac(&pswWBasisVecs[siI * S_LEN],
...
#endif
#define HR_STEP(siJ)     \
    L_G = L_mac(L_G, pswUpdatePtr[siJ], pswBits[siBitIndex]);  \
    siBitIndex = pswModNextBit[siBitIndex];

  if (C_BITS_UV_1 == siEBits)
  {
      for (siI = 0; siI < 63; siI++)
      {
          ...
          L_G = L_msu(L_G, pswUpdatePtr[6], 0x8000);
  ...
  else
  {
      for (siI = 0; siI < 255; siI++)
      {
          ...
          L_G = L_msu(L_G, pswUpdatePtr[8], 0x8000);
