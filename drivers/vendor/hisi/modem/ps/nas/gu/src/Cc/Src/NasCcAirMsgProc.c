...
{
    if (pstMsg->stBC1.IsExist && pstMsg->stBC2.IsExist && (!pstMsg->stBCRepeatInd.IsExist))
    {
        return VOS_ERR;
    }

    if (pstMsg->stLLC1.IsExist && pstMsg->stLLC2.IsExist && (!pstMsg->stLLCRepeatInd.IsExist))
    {
        return VOS_ERR;
    }

    if (pstMsg->stHLC1.IsExist && pstMsg->stHLC2.IsExist && (!pstMsg->stHLCRepeatInd.IsExist))
    {
        return VOS_ERR;
    }
...
{
    if (pstMsg->stBC1.IsExist && pstMsg->stBC2.IsExist && (!pstMsg->stRepeatInd.IsExist))
    {
        return VOS_ERR;
    }
    ...
    )
{
    ...
    NAS_CC_GetFacilityInfo(pstFacIE->aucFacility, &stFacInfo);
...
)
{
    if(((ucPi >= 1) && (ucPi <= 3))
        || ((ucPi >= 6) && (ucPi <= 20)))
    {
        NAS_CC_SetProgressFlag(entityId, VOS_TRUE);
    }
}
/*****************************************************************************
...
*/
    ...
    if (NAS_CC_CALL_STATE_U1 == enCallState)
    {
        ...
        if (NAS_CC_GetProgressFlag(entityId) != VOS_TRUE)
        {
...
)
{
    ...
    if (NAS_CC_CALL_STATE_U0_3 == enCallState)
    {
        ...
        if (NAS_IE_IS_PRESENT(&pstMsg->stSetupContainer))
        {
            ...
            NAS_CC_DecodeMsg(aucSetupContainer, 2+ pstMsg->stSetupContainer.LastOctOffset,&f_unCcDecodeBuf);
    ...
    case NAS_PROT_ERR_MANDATORY_IE_ERR:
        ...
        case NAS_CC_MSG_RELEASE_COMPLETE:
            ...
            */
            ...
            NAS_CC_ProcReleaseCompleteMsg(NAS_CC_GetEntityByTi(ucTi), (NAS_CC_MSG_RELEASE_COMPLETE_MT_STRU *)&f_unCcDecodeBuf);
            break;
        case NAS_CC_MSG_HOLD_REJ:
            ...
            */
            ...
            NAS_CC_ProcHoldRejMsg(NAS_CC_GetEntityByTi(ucTi), (NAS_CC_MSG_HOLD_REJ_STRU *)&f_unCcDecodeBuf);
            break;
        case NAS_CC_MSG_RETRIEVE_REJ:
            NAS_CC_FillCauseIe(NAS_CC_CAUSE_127, &f_unCcDecodeBuf.stRetrieveRej.stCause);
            NAS_CC_ProcRetrieveRejMsg(NAS_CC_GetEntityByTi(ucTi), (NAS_CC_MSG_RETRIEVE_REJ_STRU *)&f_unCcDecodeBuf);
            break;
